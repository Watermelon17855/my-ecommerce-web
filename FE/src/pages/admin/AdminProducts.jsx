...
import { Plus, Pencil, Trash2, X, Eye, EyeOff, Package, Tag, Image as ImageIcon, Percent } from 'lucide-react';
...
const AdminProducts = () => {
    ...
    // 1. Thêm trường originalPrice vào state
    ...
    // Hàm tính nhanh % giảm giá để hiển thị preview
    ...
    // --- LOGIC MODAL (Cập nhật reset giá gốc) ---
    const openAddModal = () => {
        setIsEditMode(false);
        setCurrentProduct({ name: '', originalPrice: '', price: '', img: '', description: '', isAvailable: true, category: '', subImages: [] });
        setIsModalOpen(true);
    };
    ...
    // ... Các hàm handleDelete, handleToggleStatus giữ nguyên ...
                ...
                <table className="w-full text-left">
                    <thead className="bg-gray-50 text-gray-400 text-[11px] uppercase tracking-wider font-bold">
                        <tr>
                            ...
                            <th className="px-6 py-4 text-center">Giá & Sale</th>
                            ...
                            return (
                                <tr key={product._id} className={`hover:bg-gray-50 transition-all ${!product.isAvailable ? 'opacity-50' : ''}`}>
                                    ...
                                    <td className="px-6 py-4 text-center">
                                        <div className="text-blue-600 font-black text-sm">{product.price?.toLocaleString()}đ</div>
                                        {discount > 0 && (
                                            <div className="flex items-center justify-center gap-1">
                                                <span className="text-gray-400 line-through text-[10px]">{product.originalPrice?.toLocaleString()}đ</span>
                                                <span className="text-red-500 text-[10px] font-bold">-{discount}%</span>
                                            </div>
                                        )}
                                    </td>
                                    <td className="px-6 py-4 text-center">
                    ...
                    <div className="bg-white rounded-[2rem] p-8 w-full max-w-xl shadow-2xl relative animate-in fade-in zoom-in duration-200 overflow-y-auto max-h-[90vh]">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4 bg-gray-50 p-4 rounded-3xl border border-gray-100">
                                <div>
                                    <label className="text-[10px] font-bold text-gray-400 uppercase ml-2">Giá gốc (VNĐ)</label>
                                    <input type="number" placeholder="Ví dụ: 1000000" value={currentProduct.originalPrice} className="w-full p-3 bg-white rounded-xl border-none focus:ring-2 focus:ring-blue-500 outline-none shadow-sm" onChange={(e) => setCurrentProduct({ ...currentProduct, originalPrice: e.target.value })} required />
                                </div>
                                <div>
                                    <label className="text-[10px] font-bold text-blue-400 uppercase ml-2">Giá sau giảm (VNĐ)</label>
                                    <input type="number" placeholder="Ví dụ: 800000" value={currentProduct.price} className="w-full p-3 bg-white rounded-xl border-none focus:ring-2 focus:ring-blue-500 outline-none shadow-sm" onChange={(e) => setCurrentProduct({ ...currentProduct, price: e.target.value })} required />
                                </div>
                                {currentProduct.originalPrice > 0 && currentProduct.price > 0 && (
                                    <div className="col-span-2 text-center">
                                        <span className="text-xs font-bold text-green-600 bg-green-50 px-3 py-1 rounded-full border border-green-100">
                                            🎉 Tiết kiệm cho khách: {getDiscountPercentage(currentProduct.originalPrice, currentProduct.price)}%
                                        </span>
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="space-y-2">
                                ...
                                {!isAddingNewCat ? (
                                    <div className="flex gap-2">
                                        <select value={currentProduct.category} onChange={(e) => setCurrentProduct({ ...currentProduct, category: e.target.value })} className="flex-1 p-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none">
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        <button type="button" className="px-4 bg-green-600 text-white rounded-xl font-bold">Lưu</button>
                            ...
                            <div className="space-y-4 p-5 bg-blue-50/50 rounded-3xl border border-blue-100">
                                ...
                                <div className="flex gap-2">
                                    <input type="text" placeholder="Dán link ảnh chi tiết vào đây..." value={currentSubLink} onChange={(e) => setCurrentSubLink(e.target.value)} className="flex-1 p-3 bg-white rounded-xl border-none focus:ring-2 focus:ring-blue-500 outline-none shadow-sm" />
                                    <button type="button" onClick={() => { if (!currentSubLink.trim()) return; setCurrentProduct({ ...currentProduct, subImages: [...(currentProduct.subImages || []), currentSubLink] }); setCurrentSubLink(""); }} className="bg-blue-600 text-white px-4 rounded-xl font-bold hover:bg-blue-700 shadow-md shadow-blue-100">Thêm</button>
                                </div>
                                <div className="flex flex-wrap gap-3">
                                    {currentProduct.subImages?.map((url, index) => (
                                        <div key={index} className="relative group w-16 h-16">
                                            ...
                                            <button type="button" onClick={() => { const filtered = currentProduct.subImages.filter((_, i) => i !== index); setCurrentProduct({ ...currentProduct, subImages: filtered }); }} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full w-5 h-5 flex items-center justify-center text-[10px] opacity-0 group-hover:opacity-100 transition-all shadow-md"><X size={10} /></button>
                                        </div>
                                    ))}
                                </div>
                            </div>
